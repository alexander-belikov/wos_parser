...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--source', help='source file name, expected extension xml.gz')
    ...
    logging.basicConfig(filename=logfile, level=log_levels[args.verbosity],
                        format='%(asctime)s %(name)-12s %(levelname)-8s %(message)s',
                        datefmt='%m-%d %H:%M')

    convert(source, target, args.chunksize, maxchunks)
